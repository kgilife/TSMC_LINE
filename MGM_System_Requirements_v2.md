...
2. **批量生成連結區塊**：提供文字輸入框 (Textarea)，讓管理者可以直接貼上從 Excel/Google Sheet 複製來的兩欄資料 (第一欄為網址編號，第二欄為使用者代碼)。解析後呼叫後端 API 進行生成。
3. **已生成連結管理區塊**：提供列表顯示所有已生成的 `User_Code` 對應 `Short_URL`，並提供「刪除」與「複製」功能。
